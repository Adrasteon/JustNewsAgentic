...
class PracticalNewsReader:
    ...
    async def initialize_option_a_lightweight_llava(self):
        ...
        try:
            ...
            logger.info(
                f"✅ LLaVA-1.5 loaded with {memory_usage.get('allocated_gb', 0):.1f}GB GPU memory"
            )
    ...
    async def initialize_option_b_blip2(self):
        ...
        try:
            ...
            memory_usage = self.get_memory_usage()
            logger.info(
                f"✅ BLIP-2 loaded with {memory_usage.get('allocated_gb', 0):.1f}GB GPU memory"
            )
    ...
    async def initialize_option_b_blip2_quantized(self):
        ...
        try:
            ...
            memory_usage = self.get_memory_usage()
            logger.info(
                f"✅ BLIP-2 quantized loaded with {memory_usage.get('allocated_gb', 0):.1f}GB GPU memory"
            )
...
async def test_practical_approach():
    ...
    try:
        ...
        try:
            ...
            print(f"✅ Analysis successful: {result['success']}")
            print(
                f"📝 Analysis preview: {result.get('analysis', 'No analysis')[:100]}..."
            )
            print(f"💾 Memory usage: {result.get('memory_usage_gb', 0):.1f}GB")
