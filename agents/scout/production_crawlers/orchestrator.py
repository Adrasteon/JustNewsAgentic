...
class ProductionCrawlerOrchestrator:
    ...
    async def crawl_site_ultra_fast(
        self, site: str, target_articles: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        if site not in self.sites:
            raise ValueError(
                f"Site '{site}' not supported. Available: {list(self.sites.keys())}"
            )
        ...
        logger.info(
            f"Starting ultra-fast crawl of {site} for {target_articles} articles"
        )
        ...
        try:
            ...
            articles_per_second = (
                len(results.get("articles", [])) / duration if duration > 0 else 0
            )
    ...
    async def crawl_site_ai_enhanced(
        self, site: str, target_articles: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Starting AI-enhanced crawl of {site} for {target_articles} articles"
        )
        ...
        try:
            ...
            return {
                ...
                "ai_analysis_count": sum(
                    1 for a in results.get("articles", []) if "ai_analysis" in a
                ),
                "timestamp": start_time.isoformat(),
            }
    ...
    async def crawl_multi_site(
        self, sites: List[str], mode: str = "ultra_fast", articles_per_site: int = 50
    ) -> List[Dict[str, Any]]:
        """
        ...
        for site in sites:
            if mode == "ultra_fast":
                ...
                raise ValueError(
                    f"Invalid mode: {mode}. Use 'ultra_fast' or 'ai_enhanced'"
                )
