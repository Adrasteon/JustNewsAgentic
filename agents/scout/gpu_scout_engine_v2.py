...
warnings.filterwarnings(
    "ignore", category=UserWarning, module="transformers.pipelines.text_classification"
)
...
class NextGenGPUScoutEngine:
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                self.tokenizers["quality_assessor"] = AutoTokenizer.from_pretrained(
                    quality_config["model_name"], trust_remote_code=True, use_fast=True
                )
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                self.tokenizers["bias_detector"] = AutoTokenizer.from_pretrained(
                    bias_config["model_name"], trust_remote_code=True, use_fast=True
                )
        ...
        if GPU_CLEANUP_AVAILABLE and self.device.startswith("cuda"):
            ...
            for pipeline_name, pipeline_obj in self.pipelines.items():
                if pipeline_obj is not None:
                    gpu_manager.register_model(
                        f"scout_v2_pipeline_{pipeline_name}", pipeline_obj
                    )

            logger.info("🧹 GPU models registered with cleanup manager")

    def classify_news_content(
        self, text: str, url: str = "", use_ensemble: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(predictions, list):
                ...
                if predictions.get("label", "").upper() in [
                    "LABEL_1",
                    "NEWS",
                    "TRUE",
                    "1",
            ...
            logger.debug(
                f"✅ AI News Classification: {result['is_news']} ({result['confidence']:.2f})"
            )
            return result
    ...
    def assess_content_quality(self, text: str, url: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"✅ AI Quality Assessment: {quality_rating} ({overall_quality:.2f})"
            )
            return result
    ...
    def analyze_sentiment(self, text: str, url: str = "") -> Dict[str, Any]:
        ...
        try:
                ...
                for pred in predictions:
                    if isinstance(pred, dict) and "label" in pred and "score" in pred:
                        ...
                        if label in ["positive", "pos", "label_2"]:
                            sentiment_scores["positive"] = max(
                                sentiment_scores["positive"], score
                            )
                        elif label in ["negative", "neg", "label_0"]:
                            sentiment_scores["negative"] = max(
                                sentiment_scores["negative"], score
                            )
                        else:
                            sentiment_scores["neutral"] = max(
            ...
            logger.debug(
                f"✅ AI Sentiment Analysis: {dominant_sentiment} ({confidence:.2f})"
            )
            return result
    ...
    def analyze_visual_content(
        self,
        image_path: str,
        prompt: str = "Describe this image and determine if it contains news-worthy content",
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            with torch.no_grad():
                output = model.generate(
                    **inputs,
                    max_new_tokens=self.model_configs["visual_analyzer"][
                        "max_new_tokens"
                    ],
                    do_sample=True,
                    temperature=0.1,
            ...
            is_news_visual = any(
                keyword in analysis.lower() for keyword in news_keywords
            )
    ...
    def comprehensive_content_analysis(
        self, text: str, url: str = "", image_path: str = None
    ) -> Dict[str, Any]:
        """
        ...
        result = {
            ...
            "models_used": [
                ...
                if self.models.get(
                    config["task"].split("_")[0] + "_" + config["task"].split("_")[1]
                )
            ],
        ...
        logger.info(
            f"✅ Comprehensive analysis completed. Scout Score: {scout_score:.2f}"
        )
        return result

    def add_training_example(
        self, task: str, text: str, label: Union[int, str], url: str = ""
    ):
        """
        ...
        if isinstance(label, str):
            if task == "news_classification":
                ...
                label = (
                    1 if label.lower() in ["bias", "biased", "true", "1", "yes"] else 0
                )
        ...
        logger.debug(
            f"📚 Added training example for {task}: {len(self.training_data[task]['texts'])} examples"
        )
    ...
    def fine_tune_model(
        ...
        if len(training_texts) < 10:
            logger.warning(
                f"⚠️ Insufficient training data for {task}: {len(training_texts)} examples"
            )
            return False

        try:
            logger.info(
                f"🏋️ Fine-tuning {task} model with {len(training_texts)} examples..."
            )
            ...
            logger.info(
                f"✅ Fine-tuning completed for {task}. Model saved to {model_save_path}"
            )
    ...
    def _enhance_with_quality_signals(
        self, base_result: Dict, text: str, url: str
    ) -> Dict:
        ...
        try:
            ...
            base_result["confidence"] = min(
                base_result["confidence"] + quality_bonus, 1.0
            )
            base_result[
                "reasoning"
            ] += f" + Quality enhancement ({quality_result.get('overall_quality', 0):.2f})"
    ...
    def _calculate_comprehensive_scout_score(
        self,
        news_analysis: Dict,
        ...
        try:
            ...
            news_score = (
                news_analysis.get("confidence", 0.0)
                if news_analysis.get("is_news", False)
                else 0.0
            )
            ...
            if dominant_sentiment == "neutral":
                ...
                dominant_sentiment in ["positive", "negative"]
    ...
    def _emergency_fallback(self, text: str, url: str) -> Dict:
        ...
        news_count = sum(
            1 for keyword in news_keywords if keyword.lower() in text.lower()
        )
    ...
    def _heuristic_bias_detection(self, text: str, url: str) -> Dict:
        ...
        bias_count = sum(
            1 for keyword in bias_keywords if keyword.lower() in text.lower()
        )
        ...
        return {
            ...
            "bias_level": (
                "high" if bias_score > 0.6 else "medium" if bias_score > 0.3 else "low"
            ),
            "confidence": 0.5,
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        for task, config in self.model_configs.items():
            model_key = (
                task.split("_")[0] + "_" + task.split("_")[1] if "_" in task else task
            )
            model_info[task] = {
                ...
                "training_examples": (
                    len(self.training_data.get(config["task"], {}).get("texts", []))
                    if self.enable_training
                    else "N/A"
                ),
            }
