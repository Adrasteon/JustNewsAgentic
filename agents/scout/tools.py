...
def search_related_articles(main_url: str, num_related: int = 3) -> List[str]:
    ...
    params = {"q": main_url, "count": num_related, "mkt": "en-US"}
...
def deep_crawl_site(*args, **kwargs):
    ...
    try:
        ...
        if intelligence_available and scout_engine and crawl_results:
            ...
            log_feedback(
                "deep_crawl_site_basic",
                {"args": args, "results_count": len(crawl_results)},
            )
...
try:
    ...
    try:
        ...
        try:
            ...
            log_feedback(
                "production_crawler_supported_sites_error",
                {"error": str(site_e), "traceback": traceback.format_exc()},
            )
        logger.info(
            f"[DIAG] ProductionCrawlerOrchestrator supported_sites at startup: {supported_sites}"
        )
        log_feedback(
            "production_crawler_supported_sites", {"supported_sites": supported_sites}
        )
...
async def production_crawl_ultra_fast(site: str, target_articles: int = 100):
    ...
    """
    logger.info(
        f"[ScoutAgent] Ultra-fast production crawl: {site} ({target_articles} articles)"
    )
...
async def production_crawl_ai_enhanced(site: str, target_articles: int = 100):
    ...
    """
    logger.info(
        f"[ScoutAgent] AI-enhanced production crawl: {site} ({target_articles} articles)"
    )
    if not PRODUCTION_CRAWLERS_AVAILABLE:
        error_msg = "Production crawlers not available"
        logger.error(error_msg)
...
def get_production_crawler_info():
    ...
    try:
        supported_sites = production_crawler.get_supported_sites()
        site_info = {
            site: production_crawler.get_site_info(site) for site in supported_sites
        }
        ...
        log_feedback(
            "get_production_crawler_info", {"sites_count": len(supported_sites)}
        )
        return result
    except Exception as e:
