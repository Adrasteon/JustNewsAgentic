...
class CriticV2Engine:
    ...
    def _load_bert_model(self):
        ...
        try:
            ...
            self.tokenizers["bert"] = BertTokenizer.from_pretrained(
                self.config.bert_model, cache_dir=self.config.cache_dir
            )
        ...
        try:
            ...
            try:
                ...
                self.tokenizers["roberta"] = RobertaTokenizer.from_pretrained(
                    self.config.roberta_model, cache_dir=self.config.cache_dir
                )
    ...
    def _load_deberta_model(self):
        ...
        try:
            ...
            self.tokenizers["deberta"] = DebertaTokenizer.from_pretrained(
                self.config.deberta_model, cache_dir=self.config.cache_dir
            )
    ...
    def _load_distilbert_model(self):
        ...
        try:
            ...
            self.tokenizers["distilbert"] = DistilBertTokenizer.from_pretrained(
                self.config.distilbert_model, cache_dir=self.config.cache_dir
            )
            ...
            logger.info(
                "✅ DistilBERT readability assessment model loaded successfully"
            )
    ...
    def _load_embedding_model(self):
        ...
        try:
            if not SENTENCE_TRANSFORMERS_AVAILABLE:
                logger.warning(
                    "SentenceTransformers not available - using basic similarity"
                )
                return
    ...
    def assess_quality_bert(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                quality_score = sum(result["score"] for result in results) / len(
                    results
                )
            else:
                quality_score = 0.5
    ...
    def evaluate_factual_consistency_deberta(
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                consistency_score = consistency_scores.get(
                    "consistent", consistency_scores.get("CONSISTENT", 0.5)
                )
            else:
            ...
            evaluation = {
                ...
                "confidence": (
                    max(consistency_scores.values()) if consistency_scores else 0.5
                ),
                "model": "deberta",
            }

            self.log_feedback(
                "evaluate_factual_consistency_deberta",
                {
                    "consistency_score": consistency_score,
    ...
    def assess_readability_distilbert(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if results and len(results) > 0:
                readability_score = sum(result["score"] for result in results) / len(
                    results
                )
    ...
    def detect_plagiarism_embeddings(
        self, text: str, reference_texts: List[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if SKLEARN_AVAILABLE:
                ...
                similarities = [
                    np.dot(text_embedding[0], ref_emb)
    ...
    def comprehensive_review(
        self, text: str, context: str = "", reference_texts: List[str] = None
    ) -> ReviewResult:
        """
        ...
        try:
            ...
            plagiarism_analysis = self.detect_plagiarism_embeddings(
            ...
            category_scores = {
                ...
                ReviewCategory.BIAS: 1.0
                - bias_analysis.get("bias_score", 0.0),  # Invert bias score
                ReviewCategory.FACTUAL: factual_analysis.get("consistency_score", 0.5),
                ReviewCategory.READABILITY: readability_analysis.get(
                    "readability_score", 50.0
                )
                / 100.0,
                ReviewCategory.ORIGINALITY: 1.0
                - plagiarism_analysis.get("similarity_score", 0.0),
            }
    ...
    def _generate_flags(
        self, bias_analysis, factual_analysis, plagiarism_analysis
    ) -> List[str]:
    ...
    def _fallback_quality_assessment(self, text):
        return {"overall_score": 0.5, "assessment": "medium", "model": "fallback"}
    ...
    def get_model_status(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "roberta": self.models.get("roberta") is not None
            or self.pipelines.get("roberta_bias") is not None,
            ...
            "total_models": sum(
                1
                for key in ["bert", "roberta", "deberta", "distilbert", "embeddings"]
                if self.models.get(key) is not None
                or self.pipelines.get(f"{key}_bias") is not None
            ),
        }
    ...
    def add_reference_texts(self, texts: List[str]):
        ...
        logger.info(
            f"Added {len(texts)} reference texts. Total: {len(self.knowledge_base)}"
        )
...
def test_critic_v2_engine():
    ...
    try:
        ...
        factual = engine.evaluate_factual_consistency_deberta(
            sample_text, sample_context
        )
        ...
        review = engine.comprehensive_review(
            sample_text, sample_context, reference_texts
        )
