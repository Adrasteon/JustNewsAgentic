    ...
    try:
        mcp_bus_client.register_agent(
            ...
            tools=[
                "critique_synthesis",
                "critique_neutrality",
                "critique_content_gpu",
                "get_critic_performance",
            ],
        )
...
def critique_synthesis(call: ToolCall):
    try:
        ...
        logger.info(
            f"Calling critique_synthesis with args: {call.args} and kwargs: {call.kwargs}"
        )
...
def critique_neutrality(call: ToolCall):
    try:
        ...
        logger.info(
            f"Calling critique_neutrality with args: {call.args} and kwargs: {call.kwargs}"
        )
...
def critique_content_gpu_endpoint(call: ToolCall):
    ...
    try:
        ...
        logger.info(
            f"Calling GPU critique with {len(call.args[0]) if call.args else 0} articles"
        )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"GPU critique failed: {e}, CPU fallback failed: {fallback_error}",
            )
