...
class GPUAcceleratedCritic:
    ...
    def _process_batch(self, batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for article in batch:
            try:
                ...
                quality_results = self.critique_pipeline(
                    full_text[:512]
                )  # Limit to model's max length
                ...
                if quality_results and isinstance(quality_results, list):
                    ...
                    scores_dict = {
                        item["label"]: item["score"] for item in quality_results[0]
                    }

                    # Primary toxicity categories to evaluate
                    primary_toxic_categories = [
                        "toxicity",
                        "severe_toxicity",
                        "insult",
                        "threat",
                    ...
                    toxic_scores = [
                        scores_dict.get(cat, 0.0) for cat in primary_toxic_categories
                    ]
                    ...
                    toxicity_details = {
                        ...
                        for cat in primary_toxic_categories
                    }
                ...
                logger.warning(
                    f"⚠️ Failed to critique article '{article.get('title', 'Unknown')}': {e}"
                )
    ...
    def _analyze_quality_classification(
        self,
        label: str,
        score: float,
        quality_score: float,
        ...
        if label == "TOXIC":
            ...
            toxic_str = (
                ", ".join(toxic_categories) if toxic_categories else "general toxicity"
            )
            critique_text = (
        ...
        if word_count < 50:
            ...
            recommendations.append(
                "Article may be too brief for comprehensive coverage"
            )
        elif word_count < 100:
    ...
    def _detect_bias_indicators(self, content: str) -> List[str]:
        ...
        """
        logger.warning(
            "⚠️ _detect_bias_indicators called - bias detection moved to Scout V2"
        )
