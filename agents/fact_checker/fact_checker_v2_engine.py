...
class FactCheckerV2Engine:
            ...
            for model_name, pipeline in self.pipelines.items():
                if pipeline is not None:
                    gpu_manager.register_model(
                        f"fact_checker_v2_{model_name}", pipeline
                    )

        # Validation and completion
        successful_models = len(
            [p for p in self.pipelines.values() if p is not None]
        ) + len([m for m in self.models.values() if m is not None])
        logger.info(
            f"✅ Fact Checker V2 Engine ready with {successful_models} AI models"
        )
    ...
    def _initialize_claim_extraction_model(self):
        ...
        try:
            ...
            logger.error(
                "❌ spaCy en_core_web_sm model not found. Download with: python -m spacy download en_core_web_sm"
            )
            self.models["claim_extraction"] = None
        except Exception as e:
    ...
    def verify_fact(self, claim: str, context: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                factual_score = max(
                    [s["score"] for s in scores if "factual" in s["label"].lower()],
                    default=0.5,
                )
    ...
    def assess_source_credibility(
        self, source_text: str, domain: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                reliability = (
                    ...
                    else ("medium" if credibility_score > 0.4 else "low")
                )
    ...
    def retrieve_evidence(
        self, query: str, evidence_database: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            evidence_embeddings = self.models["evidence_retrieval"].encode(
                evidence_database
            )

            # Calculate similarities
            similarities = self.models["evidence_retrieval"].similarity(
    ...
    def comprehensive_fact_check(
        self, article_text: str, source_url: str = ""
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            avg_verification = (
                np.mean(
                    [
                        fv["verification"]["verification_score"]
            ...
            overall_score = max(
                ...
                min(
                    1.0,
                    (avg_verification + credibility_score) / 2 - contradiction_penalty,
                ),
            )
            ...
            return {"overall_score": 0.5, "assessment": "error", "error": str(e)}
