...
class GPUAcceleratedFactChecker:
    ...
    def validate_is_news(self, content: str) -> Dict[str, Any]:
        ...
        try:
            if self.gpu_available and self.models_loaded:
                ...
                is_news = (
                    result["labels"][0] == "news article" and result["scores"][0] > 0.5
                )
                ...
                confidence = len([k for k in keywords if k in content.lower()]) / len(
                    keywords
                )
            ...
            log_feedback(
                "validate_is_news",
                {"content_length": len(content), "result": result, "method": method},
            )
    ...
    def verify_claims_batch(
        ...
        try:
            ...
            log_feedback(
                "verify_claims_error",
                {"claims": claims, "sources_count": len(sources), "error": str(e)},
            )
            return {
    ...
    def _gpu_verify_claims(
        ...
        joined_sources = "\\n".join(
            sources[:3]
        )  # Limit source length for token efficiency
        ...
        batch_size = min(
            4, len(batch_prompts)
        )  # Conservative batch size for 774M model
        ...
        for i in range(0, len(batch_prompts), batch_size):
            ...
            for claim, output in zip(batch_claims, outputs):
                ...
                if (
                    "verified" in response_lower
                    and "not verified" not in response_lower
                ):
