...
class NewsReaderV2Engine:
    ...
    def __init__(self, config: NewsReaderV2Config = None):
        ...
        logger.info(
            "✅ NewsReader V2 Engine initialized with comprehensive multi-modal capabilities"
        )
    ...
    def _load_llava_model(self):
        ...
        try:
            ...
            self.processors["llava"] = AutoTokenizer.from_pretrained(
                self.config.llava_model, cache_dir=self.config.cache_dir
            )
    ...
    def _load_ocr_engine(self):
        ...
        try:
            ...
            self.models["ocr"] = easyocr.Reader(
                self.config.ocr_languages, gpu=self.device.type == "cuda"
            )
    ...
    def _load_layout_parser(self):
        ...
        try:
            if not LAYOUT_PARSER_AVAILABLE:
                logger.warning(
                    "LayoutParser not available - using basic layout analysis"
                )
