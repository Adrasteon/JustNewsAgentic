...
try:
    ...
    logger.warning(
        "NewsReader V2 TRUE engine not available - using fallback processing"
    )
...
def get_engine():
    ...
    if _engine_instance is None and V2_ENGINE_AVAILABLE:
        if torch.cuda.is_available():
            ...
            if (
                allocated_gb > 15.0
            ):  # If >15GB already allocated, don't create new engine
                logger.error(
                    f"❌ GPU memory safety check failed: {allocated_gb:.1f}GB already allocated"
                )
        ...
        try:
            ...
            if (
                hasattr(_engine_instance, "is_llava_available")
                and not _engine_instance.is_llava_available()
            ):
                logger.warning(
                    "V2 Engine initialized but LLaVA model not available - will use fallbacks"
                )
    ...
    try:
        ...
        if engine and V2_ENGINE_AVAILABLE:
            ...
            if isinstance(content, dict) and "url" in content:
                ...
                logger.info(
                    f"🔍 Processing URL with TRUE LLaVA screenshot analysis: {url}"
                )
            ...
            log_feedback(
                "content_processed",
                {
                    ...
                    "processing_time": (
                        ...
                        else formatted_result["processing_time"]
                    ),
                    "confidence": formatted_result.get("analysis_results", {}).get(
                        "confidence_score", 0.0
                    ),
                    "models_used": formatted_result.get("v2_compliance", {}).get(
                        "models_used", 0
                    ),
                    "screenshot_based": formatted_result.get("v2_compliance", {}).get(
...
async def _process_text_content(
    ...
    result = {
        ...
        "metadata": {
            ...
            "estimated_reading_time": len(text_content.split())
            / 200,  # 200 WPM average
...
async def analyze_content_structure(
    content: str, analysis_depth: str = "comprehensive"
) -> Dict[str, Any]:
    """
...
def _create_basic_structure_analysis(content: str) -> Dict[str, Any]:
    ...
    return {
        "content_sections": [
            {"type": "full_content", "content": content[:200] + "..."}
        ],
        "hierarchy_levels": {"basic_analysis": True},
        "semantic_structure": {"fallback_mode": True},
        "quality_metrics": {"word_count": len(content.split()), "basic_analysis": True},
    }


async def extract_multimedia_content(
    content: Union[str, bytes, dict], extraction_types: List[str] = None
) -> Dict[str, Any]:
    """
...
async def extract_news_from_url(
    url: str, screenshot_path: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "headline": news_extraction.get("headline")
            or result["extracted_content"]["metadata"].get(
                "title", "Extracted Headline"
            ),
            "article": news_extraction.get("article")
            or result["extracted_content"]["main_text"],
            "success": result["status"] == "success",
            "method": "newsreader-v2-llava-screenshot",
            "processing_time": result["processing_time"],
            "url": url,
            "v2_enhanced": True,
            "screenshot_based": result.get("v2_compliance", {}).get(
                "screenshot_based", False
            ),
            "llava_processing": result.get("v2_compliance", {}).get(
                "llava_processing", False
            ),
            "screenshot_path": result.get("screenshot_info", {}).get("screenshot_path"),
            "confidence_score": result.get("analysis_results", {}).get(
                "confidence_score", 0.0
            ),
        }
...
def analyze_image_with_llava(image_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if engine and V2_ENGINE_AVAILABLE:
            ...
            # Format for legacy compatibility
            return {
                "headline": result["extracted_content"]["metadata"].get(
                    "title", "Extracted Headline"
                ),
...
async def test_newsreader_v2():
    ...
    result = await process_article_content(
        content=test_content, content_type="article", processing_mode="comprehensive"
    )
