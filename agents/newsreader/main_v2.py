...
# Import V2 tools
from .tools import analyze_image_with_llava  # Legacy compatibility
from .tools import capture_webpage_screenshot  # Legacy compatibility
from .tools import extract_news_from_url  # Legacy compatibility
from .tools import (
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if health_status.get("v2_compliance", False):
            ...
            logger.info(
                f"✅ {len(health_status.get('capabilities', []))} capabilities available"
            )
            logger.info(
                f"✅ GPU acceleration: {health_status.get('components', {}).get('gpu_available', False)}"
            )
        else:
...
def health():
    ...
    try:
        ...
        return {
            ...
            "models_available": health_status.get("components", {}).get(
                "models_loaded", 0
            ),
            "gpu_acceleration": health_status.get("components", {}).get(
                "gpu_available", False
            ),
            "engine_status": (
                "available"
                if health_status.get("engine_available", False)
                else "fallback_mode"
            ),
        }
    except Exception as e:
...
async def process_content_endpoint(call: ToolCall):
    ...
    try:
        ...
        logger.info(
            f"Content processed: type={result.get('content_type', 'unknown')}, "
...
async def analyze_structure_endpoint(call: ToolCall):
    ...
    try:
        ...
        logger.info(
            f"Structure analyzed: depth={result.get('analysis_depth', 'unknown')}, "
...
async def extract_multimedia_endpoint(call: ToolCall):
    ...
    try:
        ...
        logger.info(
            f"Multimedia extracted: types={len(result.get('extraction_types', []))}, "
            f"time={result.get('processing_time', 0):.2f}s"
        )
...
async def extract_news_endpoint(call: ToolCall):
    ...
    try:
        ...
        logger.info(
            f"Legacy news extraction: url={call.kwargs.get('url', 'unknown')}, "
...
def analyze_image_endpoint(call: ToolCall):
    ...
    try:
        ...
        logger.info(
            f"Legacy image analysis: path={call.kwargs.get('image_path', 'unknown')}, "
            f"v2_enhanced={result.get('v2_enhanced', False)}"
        )
        return result
    except Exception as e:
...
async def api_analyze_structure(request: StructureAnalysisRequest):
    ...
    try:
        return await analyze_content_structure(
            content=request.content, analysis_depth=request.analysis_depth
        )
    except Exception as e:
...
async def api_extract_multimedia(request: MultimediaExtractionRequest):
    ...
    try:
        return await extract_multimedia_content(
            content=request.content, extraction_types=request.extraction_types
        )
    except Exception as e:
    ...
    try:
        ...
        return {
            ...
            "processing_components": {
                "models_loaded": health_status.get("components", {}).get(
                    "models_loaded", 0
                ),
                "processors_available": health_status.get("components", {}).get(
                    "processors_loaded", 0
                ),
                "fallback_systems": health_status.get("components", {}).get(
                    "fallback_systems", False
                ),
            },
            "performance": {
                "gpu_available": health_status.get("components", {}).get(
                    "gpu_available", False
                ),
