...
class MemoryV2Engine:
    ...
    def _load_embedding_model(self):
        ...
        try:
            if not SENTENCE_TRANSFORMERS_AVAILABLE:
                logger.warning(
                    "SentenceTransformers not available - using basic embeddings"
                )
                return
            ...
            logger.info(
                f"✅ SentenceTransformer embedding model loaded (dim: {self.config.embedding_dim})"
            )
        ...
        try:
            ...
            try:
                ...
                self.faiss_index = faiss.IndexFlatL2(
                    384
                )  # Hard-coded dimension for SentenceTransformer
    ...
    def _initialize_postgresql(self):
        ...
        try:
            ...
            if not all([POSTGRES_HOST, POSTGRES_DB, POSTGRES_USER]):
                logger.warning(
                    "PostgreSQL environment variables not set - falling back to SQLite"
                )
                self._initialize_sqlite()
                return
            ...
            with self.postgresql_conn.cursor() as cursor:
                ...
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_memory_v2_content_type ON memory_v2_items(content_type)"
                )
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_memory_v2_created_at ON memory_v2_items(created_at)"
                )
    ...
    def generate_embedding(self, content: str) -> np.ndarray:
        ...
        try:
            ...
            self.log_feedback(
                "generate_embedding",
                {"content_length": len(content), "embedding_dim": len(embedding)},
            )
        ...
        try:
            ...
            if strategy == RetrievalStrategy.SEMANTIC:
                return self._semantic_search(
                    query, content_type, limit, similarity_threshold
                )
    ...
    def _semantic_search(
        ...
        try:
                    ...
                    if idx != -1 and distance >= similarity_threshold:
                        ...
                        if item_id and item_id in self.memory_cache:
                            ...
                            if (
                                ...
                                or memory_item.content_type == content_type
                            ):
                                results.append(
                                    SearchResult(
                                        item=memory_item,
            ...
            # Fallback to basic similarity
                ...
                results = self._fallback_semantic_search(
                    query, content_type, limit, similarity_threshold
                )
    ...
    def _keyword_search(
        self, query: str, content_type: ContentType = None, limit: int = 10
    ) -> List[SearchResult]:
        ...
        try:
            ...
            for result in keyword_results:
                ...
                if item_id in combined_results:
                    # Combine scores
                    combined_results[item_id].score += (
                        result.score * 0.3
                    )  # Keyword weight
    ...
    def _cluster_search(
        ...
        try:
            ...
            for i, (result, label) in enumerate(zip(results, cluster_labels)):
                if (
                    label not in cluster_representatives
                    or result.score > cluster_representatives[label].score
                ):
        ...
        try:
            ...
            if self.sqlite_conn is not None:
                ...
                for row in cursor.fetchall():
                    stats["content_type_distribution"][row["content_type"]] = row[
                        "count"
                    ]
...
def test_memory_v2_engine():
    ...
    try:
        ...
        semantic_results = engine.search_memory(
            search_query, RetrievalStrategy.SEMANTIC, limit=3
        )
        ...
        keyword_results = engine.search_memory(
            search_query, RetrievalStrategy.KEYWORD, limit=3
        )
        print(f"   Found {len(keyword_results)} keyword matches")

        print("🔀 Testing hybrid search...")
        hybrid_results = engine.search_memory(
            search_query, RetrievalStrategy.HYBRID, limit=3
        )
        print(f"   Found {len(hybrid_results)} hybrid matches")

        print("⏰ Testing temporal search...")
        temporal_results = engine.search_memory(
            search_query, RetrievalStrategy.TEMPORAL, limit=3
        )
        ...
        print(
            f"   Cache sizes: Memory={stats.get('memory_cache_size', 0)}, Embedding={stats.get('embedding_cache_size', 0)}"
        )
