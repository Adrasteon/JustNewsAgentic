...
FEEDBACK_LOG = os.environ.get(
    "SYNTHESIZER_V3_FEEDBACK_LOG", "./feedback_synthesizer_v3.log"
)
...
class SynthesizerV3ProductionEngine:
    ...
    def _load_embedding_model(self):
        ...
        try:
            if not SENTENCE_TRANSFORMERS_AVAILABLE:
                logger.warning(
                    "SentenceTransformers not available - clustering disabled"
                )
                return
            ...
            logger.info(
                "✅ SentenceTransformer embedding model loaded (reusable instance)"
            )
    ...
    def cluster_articles_advanced(self, article_texts: List[str]) -> Dict[str, Any]:
        ...
        try:
            if len(article_texts) < self.config.min_articles_for_advanced_clustering:
                ...
                logger.info(
                    f"Using fallback clustering for {len(article_texts)} articles (< {self.config.min_articles_for_advanced_clustering})"
                )
    ...
    def _fallback_clustering(self, texts: List[str]) -> Dict[str, Any]:
        ...
        try:
            if not self.embedding_model:
                ...
                clusters = [
                    [i for i in range(mid_point)],
                    [i for i in range(mid_point, len(texts))],
                ]
    ...
    def summarize_content_bart(self, text: str) -> str:
        ...
        try:
            ...
            if (
                len(words) < MIN_WORDS_FOR_SUMMARIZATION
                or char_count < MIN_CHARS_FOR_SUMMARIZATION
            ):
                logger.info(
                    f"Text too short for summarization ({len(words)} words, {char_count} chars) - returning original"
                )
            ...
            target_length = max(
                min(input_length // 3, 100), 20
            )  # 1/3 of input, reasonable bounds
    ...
    def neutralize_text_flan_t5(self, text: str) -> str:
        ...
        try:
            ...
            truncated_text = self._truncate_text_for_model(
                text, "flan_t5", max_tokens=400
            )
    ...
    def aggregate_cluster_content(self, article_texts: List[str]) -> Dict[str, str]:
        ...
        try:
            ...
            # FLAN-T5 refinement
            results["flan_t5_refined"] = self.refine_content_flan_t5(
            ...
            if results["flan_t5_refined"] and len(results["flan_t5_refined"]) > 50:
                ...
                results["flan_t5_neutralized"]
                and len(results["flan_t5_neutralized"]) > 50
            ):
    ...
    def _fallback_summarization(self, text: str, max_length: int = 150) -> str:
        ...
        return ". ".join(sentences[:2]) + (
            "." if not sentences[1].endswith(".") else ""
        )
    ...
    def get_model_status(self) -> Dict[str, Any]:
        ...
        status["total_models"] = sum(
            [1 for v in status.values() if isinstance(v, bool) and v]
        )
        return status
    ...
    def cluster_and_synthesize(self, article_texts: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not cluster_results.get("success", False):
                ...
                return {
                    "synthesis": aggregated.get(
                        "best_result", " ".join(article_texts[:2])
                    ),
...
def test_synthesizer_v3_production():
    ...
    print(
        f"✅ Clustering: {clustering['n_clusters']} clusters, method: {clustering['method']}"
    )
