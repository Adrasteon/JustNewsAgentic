...
def aggregate_cluster_endpoint(call: ToolCall):
    try:
        ...
        logger.info(
            f"Calling aggregate_cluster with args: {call.args} and kwargs: {call.kwargs}"
        )
...
def cluster_articles_endpoint(call: ToolCall):
    try:
        ...
        logger.info(
            f"Calling cluster_articles with args: {call.args} and kwargs: {call.kwargs}"
        )
...
def neutralize_text_endpoint(call: ToolCall):
    try:
        ...
        logger.info(
            f"Calling neutralize_text with args: {call.args} and kwargs: {call.kwargs}"
        )
...
def synthesize_news_articles_gpu_endpoint(call: ToolCall):
    ...
    try:
        ...
        try:
            from tools import aggregate_cluster, cluster_articles
            ...
            raise HTTPException(
                status_code=500,
                detail=f"GPU synthesis failed: {e}, CPU fallback failed: {fallback_error}",
            )
