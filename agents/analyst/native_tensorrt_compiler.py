...
class NativeTensorRTCompiler:
    ...
    def _convert_to_onnx(
        self, model_name: str, task_type: str, output_path: "Path" = None
    ) -> Optional[str]:
        ...
        try:
            ...
            dummy_text = (
                "This is a sample text for ONNX conversion. " * 10
            )  # Longer text
            ...
            logger.info(
                f"📐 Input shapes: {[(k, v.shape) for k, v in dummy_input.items()]}"
            )
            ...
            logger.error(
                f"📋 Model config: {getattr(model, 'config', 'No config available')}"
            )
            return None

    def _build_tensorrt_engine_from_onnx(
        self, onnx_path: Path, engine_path: Path, num_classes: int, task_name: str
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                config.set_memory_pool_limit(
                    ...
                    self.optimization_config["workspace_size"],
                )
            ...
            if (
                ...
                and self.optimization_config["precision"] == "fp16"
            ):
            ...
            if network.num_inputs > 2:
                ...
                if input2_shape[0] == -1:  # Dynamic batch dimension
                    ...
                    profile.set_shape(
                        input2.name, static_shape, static_shape, static_shape
                    )
                    logger.info(
                        f"📐 Added static {input2.name} profile: {static_shape}"
                    )

            config.add_optimization_profile(profile)

            logger.info(
                f"🎯 Optimization profile: min={min_shape}, opt={opt_shape}, max={max_shape}"
            )

            # Build engine
            logger.info(
                "⚡ Building optimized TensorRT engine... (this may take several minutes)"
            )
            ...
            logger.info(
                f"   Max Batch Size: {self.optimization_config['max_batch_size']}"
            )
            logger.info(f"   Sequence Length: {actual_seq_len}")
            logger.info(
                f"   Precision: {self.optimization_config['precision'].upper()}"
            )
...
def main():
    ...
    parser.add_argument(
        "--sentiment", action="store_true", help="Compile sentiment model only"
    )
    parser.add_argument("--bias", action="store_true", help="Compile bias model only")
    parser.add_argument(
        "--validate", action="store_true", help="Validate existing engines"
    )
    parser.add_argument(
        "--precision",
