...
class GPUAcceleratedAnalyst:
    ...
    def _initialize_gpu_models(self):
        ...
        try:
            if torch.cuda.is_available():
                ...
                logger.info(
                    f"Sentiment model device: {next(self.sentiment_analyzer.model.parameters()).device}"
                )
                logger.info(
                    f"Bias model device: {next(self.bias_detector.model.parameters()).device}"
                )
    ...
    def score_sentiment_batch_gpu(self, texts: List[str]) -> List[Optional[float]]:
        ...
        try:
            ...
            logger.info(
                f"✅ GPU batch sentiment: {batch_size} articles in {processing_time:.3f}s ({rate:.1f} articles/sec)"
            )
    ...
    def score_bias_batch_gpu(self, texts: List[str]) -> List[Optional[float]]:
        ...
        try:
            ...
            logger.info(
                f"✅ GPU batch bias: {batch_size} articles in {processing_time:.3f}s ({rate:.1f} articles/sec)"
            )
            return bias_scores
...
class DockerModelClient:
    ...
    def _check_availability(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "model", "list"], capture_output=True, text=True, timeout=10
            )
            ...
            if result.returncode == 0 and self.model_name in result.stdout:
                ...
                logger.warning(
                    f"Model {self.model_name} not found in Docker Model Runner"
                )
                return False
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                self._log_performance(
                    "docker_model_query", elapsed_time, len(prompt), len(response)
                )
    ...
    def _log_performance(
        self, operation: str, elapsed_time: float, input_length: int, output_length: int
    ):
    ...
    def _setup_fallback(self):
        ...
        try:
            ...
            local_model_path = Path(
                os.getenv(
                    "LOCAL_MODEL_PATH",
                    Path.home().joinpath("mistral_models", "7B-Instruct-v0.3"),
                )
            )
            ...
            if (
                ...
                logger.info(
                    f"V3 model found at {local_model_path} but loading DISABLED for stability"
                )
            else:
...
def log_feedback(event: str, details: dict, include_source: Optional[str] = None):
    ...
    with open(FEEDBACK_LOG, "a", encoding="utf-8") as f:
        f.write(
            f"{datetime.utcnow().isoformat()}\t{event}\t{json.dumps(enhanced_details)}\n"
        )
...
def score_bias(text: str) -> float:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"GPU bias analysis failed, falling back to Docker: {gpu_error}"
            )
            ...
            response, source = manager.query_with_fallback(
        ...
        log_feedback(
            "score_bias_error", {"text": text[:100], "error": str(e), "success": False}
        )
        return 0.5
...
def score_sentiment(text: str) -> float:
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                f"GPU sentiment analysis failed, falling back to Docker: {gpu_error}"
            )
        ...
        log_feedback(
            "score_sentiment_error",
            {"text": text[:100], "error": str(e), "success": False},
        )
        return 0.5
...
def score_sentiment_batch(texts: List[str]) -> List[float]:
    ...
    try:
        ...
        if gpu_analyst.models_loaded:
            ...
            if failed_indices:
                logger.warning(
                    f"🔄 {len(failed_indices)} articles failed batch processing, falling back individually"
                )
            ...
            logger.info(
                f"✅ BATCH sentiment completed: {len(texts)} articles in {processing_time:.3f}s ({rate:.1f} articles/sec)"
            )
        ...
        logger.warning(
            f"🔄 GPU batch sentiment failed, falling back to individual processing: {gpu_error}"
        )
    ...
    rate = len(texts) / processing_time if processing_time > 0 else 0
    logger.info(
        f"✅ Individual sentiment completed: {len(texts)} articles in {processing_time:.3f}s ({rate:.1f} articles/sec)"
    )
...
def score_bias_batch(texts: List[str]) -> List[float]:
    ...
    try:
        ...
        if gpu_analyst.models_loaded:
            ...
            logger.info(
                f"✅ BATCH bias completed: {len(texts)} articles in {processing_time:.3f}s ({rate:.1f} articles/sec)"
            )
        ...
        logger.warning(
            f"🔄 GPU batch bias failed, falling back to individual processing: {gpu_error}"
        )
    ...
    rate = len(texts) / processing_time if processing_time > 0 else 0
    logger.info(
        f"✅ Individual bias completed: {len(texts)} articles in {processing_time:.3f}s ({rate:.1f} articles/sec)"
    )
        ...
        if response:
            for item in response.split(","):
                ...
                if (
                    item
                    and len(item) > 1
                    and not item.lower().startswith(("here", "the", "entities", "and"))
                ):
...
def benchmark_performance(
    ...
    results["benchmarks"] = {
        "bias_scoring": {"score": bias_score, "time_seconds": bias_time},
        "sentiment_scoring": {"score": sentiment_score, "time_seconds": sentiment_time},
    ...
    logger.info(
        f"Benchmark completed in {results['benchmarks']['total_time']:.2f} seconds"
    )
    return results
