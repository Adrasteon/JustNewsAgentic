...
class NativeTensorRTInferenceEngine:
    ...
    def __init__(
        self,
        engines_dir: str = "agents/analyst/tensorrt_engines",
    ...
    def _load_engines(self):
        ...
        try:
            ...
            if sentiment_engine_path.exists() and sentiment_metadata_path.exists():
                ...
                logger.info(
                    f"   Max Batch Size: {metadata.get('max_batch_size', 'N/A')}"
                )
            ...
            if self.engines:
                logger.info(
                    f"✅ Native TensorRT engines loaded: {list(self.engines.keys())}"
                )
            else:
    ...
    def _initialize_engines(self):
        ...
        try:
            ...
            self._load_engine(
                "sentiment",
                "native_sentiment_roberta.engine",
                "cardiffnlp/twitter-roberta-base-sentiment-latest",
            )

            # Load bias engine
            self._load_engine("bias", "native_bias_bert.engine", "unitary/toxic-bert")
    ...
    def _load_engine(self, task: str, engine_filename: str, model_name: str):
        ...
        try:
            ...
            metadata_path = self.engines_dir / (
                engine_filename.replace(".engine", ".json")
            )
            ...
            logger.info(
                f"   Max Batch Size: {metadata.get('max_batch_size', 'unknown')}"
            )
    ...
    def score_sentiment_native(self, text: str) -> Optional[float]:
        ...
        try:
            ...
            if result is not None:
                ...
                logger.debug(
                    f"⚡ Native sentiment: {score:.3f} ({processing_time*1000:.1f}ms)"
                )
                return score
            else:
                return self._fallback_sentiment(text)
    ...
    def score_bias_native(self, text: str) -> Optional[float]:
        ...
        try:
            ...
            if result is not None:
                ...
                logger.debug(
                    f"⚡ Native bias: {score:.3f} ({processing_time*1000:.1f}ms)"
                )
                return score
            else:
                return self._fallback_bias(text)
    ...
    def score_sentiment_batch_native(self, texts: List[str]) -> List[Optional[float]]:
        ...
        try:
            ...
            if results is not None:
                ...
                logger.info(
                    f"⚡ Native batch sentiment: {len(texts)} articles ({articles_per_sec:.1f} articles/sec)"
                )
    ...
    def score_bias_batch_native(self, texts: List[str]) -> List[Optional[float]]:
        ...
        try:
            ...
            if results is not None:
                ...
                articles_per_sec = len(texts) / processing_time
                logger.info(
                    f"⚡ Native batch bias: {len(texts)} articles ({articles_per_sec:.1f} articles/sec)"
                )
    ...
    def _run_native_inference(self, text: str, task: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            needs_token_type_ids = "input.3" in [
                self.engines[task].get_tensor_name(i)
    ...
    def _run_native_batch_inference(
        ...
        try:
            ...
            if batch_size > max_batch:
                logger.warning(
                    f"⚠️  Batch size {batch_size} exceeds max {max_batch}, splitting"
                )
                # Process in chunks
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if stats["native_requests"] > 0:
            ...
            stats["native_articles_per_sec"] = (
                stats["native_requests"] / stats["native_time"]
            )
    ...
    def cleanup(self):
        ...
        try:
            ...
            if (
                hasattr(self, "context_created")
                and self.context_created
                ...
                if self.cuda_context is not None:
                    try:
                        ...
                        if (
                            ...
                            and current_context.handle == self.cuda_context.handle
                        ):
...
if __name__ == "__main__":
    ...
    # Test inference
    test_text = (
        "This is fantastic news about the market performing exceptionally well today!"
    )
