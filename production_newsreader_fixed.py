...
class ProductionNewsReader:
    ...
    def initialize_with_fallback_strategy(self):
    ...
    async def analyze_news_content(self, image: Image.Image, context: str = "", batch_size: int = 1) -> Optional[str]:
        """
        ...
        try:
            ...
            if self.model_type == "production_llava":
                ...
                inputs = self.processor(
                    text=prompt,
                    images=images if batch_size > 1 else image,
                    return_tensors="pt"
                ).to(self.device)
                ...
                images = [image] * batch_size
                inputs = self.processor(images if batch_size > 1 else image, prompt, return_tensors="pt").to(self.device)
            ...
            # Release GPU memory after each analysis
            self.cleanup()
